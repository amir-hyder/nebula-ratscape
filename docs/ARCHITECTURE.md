...
The Flutter `engine/reroute_engine.dart` is a pure-Dart reference implementation of the coordinator's decision step: relevance against remaining legs, actionable position (a child aboard a vehicle cannot use a route that starts from the origin), child-suitability limits, alert-threshold suppression, and the decision actions (keep, info, reroute, delay, leave earlier, hold aboard, no route). Port its rules and tests to the TypeScript coordinator rather than re-deriving them.
